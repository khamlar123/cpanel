...
@Component({
  selector: 'app-statistics-add',
  templateUrl: './statistics-add.component.html',
  styleUrls: ['./statistics-add.component.css']
})
export class StatisticsAddComponent implements OnInit {
  ...
  addFunc(){
    ...
    this.modal.stEngLink = map2.map((m) => m);
      this.api.addStatistics(this.modal, 'addStatistics').subscribe(res => {
          if(res.status === '1'){
            alert('Add Data Successfully.');
            this.route.navigate(['/main/Manage/Statistics2/List']);
          }
